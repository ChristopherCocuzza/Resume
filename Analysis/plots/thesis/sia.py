...
def plot_pion(wdir,kc):
    ...
    filename = '%s/gallery/SIA-pion.png'%cwd
    py.savefig(filename)
...
def plot_kaon(wdir,kc):
    ...
    #py.tight_layout()
    py.subplots_adjust(left=0.07, bottom=0.05, right=0.99, top=0.99, wspace=0.05, hspace=0.1)
    filename = '%s/gallery/SIA-kaon.png'%cwd
    py.savefig(filename)
...
def plot_hadron(wdir,kc):
    ...
    for idx in sia_h_idx:
        ...
        for _ in idx:
            ...
            if _==4011: z = np.array(tab['z']) + 0.02
            else:       z = np.array(tab['z'])
            ax.errorbar(z,exp/thy,yerr=alpha/thy,fmt='k.')
    ...
    filename = '%s/gallery/SIA-hadron.png'%cwd
    py.savefig(filename)
