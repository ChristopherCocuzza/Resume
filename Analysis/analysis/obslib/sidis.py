...
def plot_pion(wdir,kc):
    ...
    for xrange in X:
        ...
        if cnt==1:
            ax.set_ylabel(r'\boldmath$dM^{h}/dz_h\times 2^i$',size=30)
    ...
    filename = '%s/gallery/sidis-pion.png'%wdir
    py.savefig(filename)
...
def plot_kaon(wdir,kc):
    ...
    py.subplots_adjust(left=0.08, bottom=0.05, right=0.98, top=0.98, wspace=0, hspace=0)
    checkdir('%s/gallery'%wdir)
    filename = '%s/gallery/sidis-kaon.png'%wdir
    py.savefig(filename)
...
def plot_hadron(wdir,kc):
    ...
    py.subplots_adjust(left=0.08, bottom=0.05, right=0.98, top=0.98, wspace=0, hspace=0)
    checkdir('%s/gallery'%wdir)
    filename = '%s/gallery/sidis-hadron.png'%wdir
    py.savefig(filename)
    print('Saving SIDIS pion figure to %s'%filename)
