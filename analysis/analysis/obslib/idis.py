...
def get_details(exp):
    ...
    if exp=='JLab dp':       color, marker, ms = 'firebrick', 'o', 5
    return color,marker,ms

def plot_proton(wdir, data, kc, istep):

    if 10020 not in data: return
    if 10010 not in data: return
    if 10016 not in data: return
    if 10003 not in data: return
    if 10026 not in data: return
    if 10027 not in data: return
    if 10028 not in data: return
    if 10029 not in data: return
    if 10030 not in data: return
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-proton-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-proton-%d.png'%(wdir,istep))
    py.close()
...
def plot_deuteron(wdir, data, kc, istep):
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-deuteron-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-deuteron-%d.png'%(wdir,istep))
    py.close()
...
def plot_CC(wdir, data, kc, istep):
    ...
    py.savefig('%s/gallery/dis-CC-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-CC-%d.png'%(wdir,istep))
    py.close()

def plot_marathon(wdir, data, kc, istep):
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-marathon-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-marathon-%d.png'%(wdir,istep))
    py.close()

def plot_marathon2(wdir, data, kc, istep):
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-marathon2-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-marathon2-%d.png'%(wdir,istep))
    py.close()

def plot_JLab_Hall_C_E03_103(wdir, data, kc, istep):
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-JLab-Hall-C-E03-103-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-JLab-Hall-C-E03-103-%d.png'%(wdir,istep))
    py.close()

def plot_JLab_Hall_C_E12_10_002(wdir, data, kc, istep):
    ...
    for ax in [ax01,ax11,ax12,ax21,ax22]:
        ...
        ax.yaxis.set_tick_params(which = 'minor', length = 2.5)

        ax.xaxis.set_tick_params(which = 'major', length = 5)
        ax.xaxis.set_tick_params(which = 'minor', length = 2.5)
        minorLocator = MultipleLocator(0.02)
        majorLocator = MultipleLocator(0.1)
    ...
    #ax11.text(0.05, 0.90, r'\textrm{\textbf{MARATHON}}', transform = ax11.transAxes, size = 30)
    ax01.text(1.10, 0.30, r'\boldmath$F_2^{D}/F_2^{p}$', transform = ax01.transAxes, size = 60)

    ax01.text(0.05, 0.90, r'$3.39 < Q^2 < 8.25;  \theta_c = 21^{\circ}$', transform = ax01.transAxes, size = 30)
    ax11.text(0.05, 0.90, r'$4.41 < Q^2 < 9.91;  \theta_c = 25^{\circ}$', transform = ax11.transAxes, size = 30)
    ax12.text(0.05, 0.90, r'$4.73 < Q^2 < 11.06; \theta_c = 29^{\circ}$', transform = ax12.transAxes, size = 30)
    ax21.text(0.05, 0.90, r'$5.13 < Q^2 < 12.09; \theta_c = 33^{\circ}$', transform = ax21.transAxes, size = 30)
    ax22.text(0.05, 0.90, r'$5.46 < Q^2 < 13.40; \theta_c = 39^{\circ}$', transform = ax22.transAxes, size = 30)


    handles, labels = [],[]
    handles.append(hand['JLab dp'])
    handles.append((thy_band,thy_plot))
    labels.append(r'\textbf{\textrm{JLab Hall C}}')
    labels.append(r'\textbf{\textrm{JAM}}')
    ax01.legend(handles,labels,loc=(1.05,0.05), fontsize = 30, frameon = 0, handletextpad = 0.3, handlelength = 1.0)


    py.tight_layout()
    py.subplots_adjust(hspace=0,wspace=0,left=0.04,right=0.99)
    py.savefig('%s/gallery/dis-JLab-Hall-C-E12-10-002-%d.png' % (wdir, istep))
    print('Saving figure to %s/gallery/dis-JLab-Hall-C-E12-10-002-%d.png'%(wdir,istep))
    py.close()
...
def plot_obs(wdir, kc):
    ...
    plot_marathon(wdir, data, kc, istep)
    plot_marathon2(wdir, data, kc, istep)
    plot_JLab_Hall_C_E03_103(wdir, data, kc, istep)
    plot_JLab_Hall_C_E12_10_002(wdir, data, kc, istep)
