...
def load_data(W2cut = 3.5, pW2cut=10):
...
def plot_kin_upol(data):
    ...
    for exp in data:
        ...
        for idx in data[exp]:
            ...
            if exp == 'idis':
                ...
                elif idx==10042: s,marker,color,label  = 40,'*','green', 'E12-10-002'
                ...
                elif idx == 20002: s, marker,color,label  = 70, 'o','red'   ,'SeaQuest'
    ...
    ax12 .tick_params(axis='both',which='both',top=True,right=False,labelleft=False,direction='in',labelsize=30)
    ...
    ax12.set_xlabel(r'\boldmath$z_h$',size=50)
...
def plot_kin_pol(data):
    ...
    labels.append(r'\textbf{\textrm{RHIC \boldmath$W/Z$}}')
    labels.append(r'\boldmath$W^2 = 10~{\rm GeV}^2$')
    ax11.legend(handles,labels,loc='upper left',fontsize=fs,frameon=False, handlelength = 0.95, handletextpad = 0.1)
    ...
    labels.append(r'\textbf{\textrm{Pol. SIDIS (\boldmath$\pi, K, h$)}}')
    labels.append(r'\textbf{\textrm{SIA (\boldmath$\pi, K, h$)}}')
    ax12.legend(handles,labels,loc='lower right',fontsize=fs,frameon=False, handlelength = 1.0, handletextpad = 0.1, ncol=1, columnspacing = 1.0)
